...
<projects>
  ...
  CRITICAL - Cross-subdomain cookie fix (do NOT revert):
  Frontend (mimgzqiulwfs.dev.vibecode.run) and backend (preview-*.dev.vibecode.run) are on different subdomains.
  The auth.ts MUST have:
    crossSubDomainCookies: { enabled: true, domain: ".dev.vibecode.run" }  (derived dynamically from BACKEND_URL)
    defaultCookieAttributes: { sameSite: "none", secure: true, httpOnly: true }
  Without this, login sets a cookie on the backend domain but the frontend cannot read it → redirect loop.
  The cookie domain is computed dynamically in auth.ts via getCookieDomain(env.BACKEND_URL).
</projects>
